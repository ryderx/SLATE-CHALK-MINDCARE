...
export async function createPost(data: PostFormData): Promise<Post> {
  ...
  const newPost: Post = {
    ...
    // Ensure dates are actual Date objects if needed downstream, otherwise strings are fine for JSON
    createdAt: new Date(),
    updatedAt: new Date(),
  };
  posts.push(newPost);
  console.log('[Blog Data API] Post created:', newPost.slug, newPost.id);
...
export async function updatePost(slug: string, data: PostFormData): Promise<Post | undefined> {
  ...
  console.log(`[Blog Data API] Post updated: old slug ${slug}, new slug ${newSlug}, id ${updatedPost.id}`);
